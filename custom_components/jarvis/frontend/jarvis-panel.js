 ...
 * v7.18.0 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.18.0) ----------
  ...
  // ----- generalized roofs over the derived footprint (v7.18.0) -----
  ...
  // the house and low at the outer edge, split into two door panels (v7.18.0).
  ...
  // A door on a footprint wall, open (swung) or closed, for placed exterior/cellar doors (v7.18.0).
  ...
  // between the two rooms the wall separates (v7.18.0).
  ...
  // editor's footprint + rooms + home type (v7.18.0).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    if (rt === 'flat') roof = flatRoofOver(L, GL, minx, miny, maxx, maxy, ztop);
      ...
      roof = (rt === 'gambrel') ? gambrelRoofOver(L, GL, hx0, miny, hx1, maxy, eave, minRise) : gableRoofOver(L, GL, hx0, miny, hx1, maxy, eave, minRise);
      gableRoofOver(L, GL, gx0, miny, gx1, maxy, _oneFloorTop(), 0, 0.20);   // attached garage: low, shallow-pitch roof well below the 2nd floor
    } else roof = (rt === 'gambrel') ? gambrelRoofOver(L, GL, minx, miny, maxx, maxy, eave, minRise) : gableRoofOver(L, GL, minx, miny, maxx, maxy, eave, minRise);
    ...
    // the slope (v7.18.0). Front/back 2F glazing stays lower: that face is roof, so it reads
    ...
    ((opts.elements && opts.elements['2f']) || []).filter(function (e) { return e.type === 'window'; }).forEach(function (e) {
      ...
      if (e.wall === 'left' || e.wall === 'right') {
        ...
        winX(L, GL, xg, yc - w / 2, yc + w / 2, zLo, zHi, d2, true, e.wall === 'left' ? -0.06 : 0.06);
      } else {
        var cx2 = h2.x0 + p * (h2.x1 - h2.x0), yf = e.wall === 'front' ? h2.y0 + 3.5 : h2.y1 - 3.5;
        winY(L, GL, yf, cx2 - w / 2, cx2 + w / 2, 10.8, 13.8, d2, true, e.wall === 'front' ? -0.3 : 0.3);
      }
    });

    // basement exterior openings — walkout doors / egress windows at grade (v7.18.0)
    var hb = null;
  ...
  function buildFromPlan(opts) {
    ...
    if (floor !== 'all') {
      ...
      draw(pf);
      // interior doors on this floor's rooms, open/closed from their sensor (v7.18.0)
      var zf = _planZ(pf);
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.18.0)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.18.0)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.18.0 fallback chain)
    ...
    // Real-time entity subscriptions (v7.18.0) — a native state_changed feed
    ...
    // Sparklines (v7.18.0) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.18.0) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.18.0: goals card. Also fixes suggestions, which _data() never
  ...
  // Placed openings (windows/doors) — v7.18.0. Parallel to the room plan.
  _getFloorElements() {
  ...
  _planGeometry(floor) {
    ...
    const walls = [], gaps = [];
    rooms.forEach(r => {
      if (r.type === 'stairs' || r.type === 'door') return;   // open — no blocking walls
      const x0 = r.x, y0 = r.y, x1 = r.x + r.w, y1 = r.y + r.h;
      walls.push({ x0, y0, x1, y1: y0 });   // front
      walls.push({ x0, y0: y1, x1, y1 });   // back
      walls.push({ x0, y0, x1: x0, y1 });   // left
      walls.push({ x0: x1, y0, x1, y1 });   // right
    });
    let fb = null;
  ...
  _computeCoverage(floor, cam, geo) {
    ...
    plan.rooms.forEach(r => {
      ...
      let hit = 0, tot = 0; const nx = 4, ny = 4;
      for (let i = 0; i < nx; i++) for (let j = 0; j < ny; j++) {
        ...
        const diff = Math.abs(((a - ang + 540) % 360) - 180);
        if (diff > half) continue;
        if (this._losClear(cx, cy, px, py, geo)) hit++;
      }
      if (hit > 0) cov[r.name] = Math.round(hit / tot * 100) / 100;
    });
    return cov;
  }
  _coneD(cam) {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.18.0).
  _planToFeet(plan) {
    const FT = 0.2, out = {};
  ...
  // Placed openings -> feet, with open/closed resolved from each mapped sensor (v7.18.0).
  _elementsToFeet(raw) {
  ...
  // as you edit, before saving (v7.18.0).
  ...
  // Per-bay garage door open/closed from the split door_mapping slots (v7.18.0).
  _house3dGarage() {
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.18.0).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.18.0): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.18.0) — rebuild it
  ...
  // ---- Floor-plan real dimensions (v7.18.0) ----
  ...
  _renderCameras(floor) {
    ...
    const rows = cams.map((c, i) => {
      ...
      const covLine = order.length
        ? '<div class="cam-cov">sees: ' + order.map(rn => '<span class="cam-cov-room' + (cov[rn] >= 0.7 ? ' full' : '') + '">' + this._esc(rn) + ' ' + Math.round(cov[rn] * 100) + '%</span>').join(' \u00b7 ') + '</div>'
      ...
      const row = '<div class="cam-row" data-ci="' + i + '">'
        ...
        + '</div>';
      return '<div class="cam-item">' + row + covLine + '</div>';
    }).join('');
    return '<div class="fp-cameras">'
      ...
      + '<div class="op-add"><button class="ctrl" id="cam-add">+ Camera</button></div>'
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // clipped (v7.18.0). Drag uses getScreenCTM so it adapts to any viewBox.
    ...
    // placed openings as wall markers (v7.18.0)
    ...
    // Cameras — icon + FOV cone (Phase 1: placement; wall-clipping is Phase 2).
    var _cams = [];
    ...
    for (var _ci = 0; _ci < _cams.length; _ci++) {
      ...
      svg += '<g class="fp-cam" data-cam-idx="' + _ci + '">'
        + '<path class="fp-cam-cone" d="' + this._coneD(_cam) + '" fill="' + (_out ? 'rgba(255,170,60,0.10)' : 'rgba(255,80,80,0.10)') + '" stroke="' + (_out ? 'rgba(255,185,90,0.6)' : 'rgba(255,110,110,0.6)') + '" stroke-width="0.7" pointer-events="none"/>'
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.18.0: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.18.0) -->
...
${this._renderRoutineLearning(d)}
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.18.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.18.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.18.0 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
      ...
      <!-- INTRUSION LOG + TRAINING (v7.18.0) -->
      <div class="panel">
        <div class="head">
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.18.0: approval now installs the automation into HA directly.
    ...
    // Routine learning: add/remove specific opt-in entities (v7.18.0)
    ...
    // Voice Confirmation: announce test (v7.18.0)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.18.0)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.18.0)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.18.0).
  ...
  _wireFloorPlanControls() {
    ...
    if (fpSave) {
      fpSave.addEventListener("click", async () => {
        ...
        try {
          ...
          this._editingPlan = null; this._editingElements = null; this._editingCameras = null;
    ...
    // Openings (windows/doors) — add / edit / remove (v7.18.0)
    ...
    this.shadowRoot.querySelectorAll('.cam-field').forEach(f => {
      ...
      f.addEventListener('input', () => {
        ...
        if (g) { const cone = g.querySelector('.fp-cam-cone'); if (cone) cone.setAttribute('d', this._coneD(cam)); }
      });
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.18.0)
  ...
  _wireFloorPlanDrag() {
    ...
    svgEl.querySelectorAll(".fp-cam").forEach(g => {
      ...
      if (dot) dot.addEventListener("mousedown", (e) => {
        ...
        if (!cam) return;
        const pt = svgPoint(e);
        dragging = { camIdx: ci, startX: pt.x, startY: pt.y, origX: cam.x, origY: cam.y, camera: true };
      });
      g.addEventListener("contextmenu", (e) => {
        e.preventDefault();
        const ci = parseInt(g.getAttribute("data-cam-idx"));
    ...
    svgEl.addEventListener("mousemove", (e) => {
      ...
      if (dragging.camera) {
        ...
        if (gc) {
          ...
          const cone = gc.querySelector('.fp-cam-cone'); if (cone) cone.setAttribute('d', self._coneD(cam));
    ...
    const endDrag = () => {
      if (dragging) { const wasCam = dragging.camera; dragging = null; if (wasCam) self._rerenderFloorEditor(); else redraw(); }
    };
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.18.0): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.18.0).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.18.0): MJPEG stream → proxy stills →
        ...
        // v7.18.0: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.18.0: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v7.18.0) ──
  ...
  // ── Optional ChromaDB vector backend — v7.18.0 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.18.0).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.18.0: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.18.0: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.18.0) */
  ...
  /* AREA DETAIL DRILL-DOWN (v7.18.0) */
  .area-detail-overlay {
