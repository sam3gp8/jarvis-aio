 ...
 * v7.85.4 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.85.4) ----------
  ...
  // One rectangle for a rectangular footprint; several for an L/T. (v7.85.4)
  // x-intervals of an orthogonal room polygon at scanline y (bay-aware decomposition, v7.85.4).
  ...
  function extWallsPoly(L, rooms, z0, z1) {
    ...
    // ONE stroked quad, not one per room edge (that seam was the "break") (v7.85.4).
  ...
  // ----- generalized roofs over the derived footprint (v7.85.4) -----
  ...
  // the house and low at the outer edge, split into two door panels (v7.85.4).
  ...
  // A door on a footprint wall, open (swung) or closed, for placed exterior/cellar doors (v7.85.4).
  ...
  // between the two rooms the wall separates (v7.85.4).
  ...
  // editor's footprint + rooms + home type (v7.85.4).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    if (rt !== 'flat') {
      ...
      // roof — so each dormer lights for ITS room, not the whole floor (v7.85.4).
    ...
    // the slope (v7.85.4). Front/back 2F glazing stays lower: that face is roof, so it reads
    ...
    // basement exterior openings — walkout doors / egress windows at grade (v7.85.4)
    var hb = null;
  ...
  function buildFromPlan(opts) {
    ...
    if (floor !== 'all') {
      ...
      draw(pf);
      // interior doors on this floor's rooms, open/closed from their sensor (v7.85.4)
      var zf = _planZ(pf);
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.85.4)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.85.4)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.85.4 fallback chain)
    ...
    // Real-time entity subscriptions (v7.85.4) — a native state_changed feed
    ...
    // Sparklines (v7.85.4) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.85.4) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.85.4: goals card. Also fixes suggestions, which _data() never
  ...
  // ── UI localization (v7.85.4) ── post-render text swap. The panel renders in
  ...
  // Placed openings (windows/doors) — v7.85.4. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.85.4).
  _planToFeet(plan) {
    const FT = 0.2, out = {};
  ...
  // Align nearly-touching room edges (3D only) so exterior walls render seamless (v7.85.4).
  _snapFeet(out) {
    const TOL = 1.0;   // feet
  ...
  // Placed openings -> feet, with open/closed resolved from each mapped sensor (v7.85.4).
  _elementsToFeet(raw) {
  ...
  // as you edit, before saving (v7.85.4).
  ...
  // Per-bay garage door open/closed from the split door_mapping slots (v7.85.4).
  _house3dGarage() {
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.85.4).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.85.4): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.85.4) — rebuild it
  ...
  // ---- Floor-plan real dimensions (v7.85.4) ----
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // clipped (v7.85.4). Drag uses getScreenCTM so it adapts to any viewBox.
    ...
    // rooms can be kept within it (v7.85.4). Enclosed rooms only (not outdoor zones).
    ...
    // placed openings as wall markers (v7.85.4)
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.85.4: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.85.4) -->
...
${this._renderExcludedEntities(d)}
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.85.4) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.85.4) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.85.4 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
      ...
      <!-- INTRUSION / SECURITY + LOG moved to their own tab (v7.85.4) -->

      <!-- VOICE CONFIRMATION (v7.85.4) -->
      <div class="panel">
        <div class="head">
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.85.4: approval now installs the automation into HA directly.
    ...
    // Routine learning: add/remove specific opt-in entities (v7.85.4)
    ...
    // Voice Confirmation: announce test (v7.85.4)
    // Manual "Analyze Now" — force a pattern-analysis pass (bypasses only the
    // 6h throttle, not the data gate) and show the outcome. (v7.85.4)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.85.4)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.85.4)
    ...
    // matching translation file and re-renders so the switch is immediate (v7.85.4).
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.85.4).
  ...
  _wireFloorPlanControls() {
    ...
    // Openings (windows/doors) — add / edit / remove (v7.85.4)
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.85.4)
  ...
  _wireFloorPlanDrag() {
    ...
    svgEl.addEventListener("mousemove", (e) => {
      ...
      if (dragging.resize) {
        ...
        // the garage (x<0) and in front of the home (y<0). (v7.85.4)
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.85.4): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.85.4).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.85.4): MJPEG stream → proxy stills →
        ...
        // v7.85.4: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.85.4: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── Intrusion Log + training (v7.85.4) ──
  ...
  // ── System Diagnostics — core service health (v7.85.4) ──
  ...
  // ── Optional ChromaDB vector backend — v7.85.4 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.85.4).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.85.4: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.85.4: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.85.4) */
  ...
  /* AREA DETAIL DRILL-DOWN (v7.85.4) */
  .area-detail-overlay {
