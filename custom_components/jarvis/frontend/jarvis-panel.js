 ...
 * v7.29.0 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.29.0) ----------
  ...
  // One rectangle for a rectangular footprint; several for an L/T. (v7.29.0)
  function _footprintMasses(rooms) {
    ...
    var ys = [];
    encl.forEach(function (r) { ys.push(r.y); ys.push(r.y + r.d); });
    ...
    for (var i = 0; i < ys.length - 1; i++) {
      ...
      encl.forEach(function (r) { if (r.y <= ym && r.y + r.d >= ym) ivs.push([r.x, r.x + r.w]); });
  ...
  function extWallsPoly(L, rooms, z0, z1) {
    ...
    // ONE stroked quad, not one per room edge (that seam was the "break") (v7.29.0).
  ...
  // ----- generalized roofs over the derived footprint (v7.29.0) -----
  ...
  // the house and low at the outer edge, split into two door panels (v7.29.0).
  ...
  // A door on a footprint wall, open (swung) or closed, for placed exterior/cellar doors (v7.29.0).
  ...
  // between the two rooms the wall separates (v7.29.0).
  ...
  // editor's footprint + rooms + home type (v7.29.0).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    if (rt !== 'flat') {
      ...
      // roof — so each dormer lights for ITS room, not the whole floor (v7.29.0).
    ...
    // the slope (v7.29.0). Front/back 2F glazing stays lower: that face is roof, so it reads
    ...
    // basement exterior openings — walkout doors / egress windows at grade (v7.29.0)
    var hb = null;
  ...
  function buildFromPlan(opts) {
    ...
    if (floor !== 'all') {
      ...
      draw(pf);
      // interior doors on this floor's rooms, open/closed from their sensor (v7.29.0)
      var zf = _planZ(pf);
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.29.0)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.29.0)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.29.0 fallback chain)
    ...
    // Real-time entity subscriptions (v7.29.0) — a native state_changed feed
    ...
    // Sparklines (v7.29.0) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.29.0) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.29.0: goals card. Also fixes suggestions, which _data() never
  ...
  // Placed openings (windows/doors) — v7.29.0. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.29.0).
  _planToFeet(plan) {
    const FT = 0.2, out = {};
  ...
  // Align nearly-touching room edges (3D only) so exterior walls render seamless (v7.29.0).
  _snapFeet(out) {
    const TOL = 1.0;   // feet
  ...
  // Placed openings -> feet, with open/closed resolved from each mapped sensor (v7.29.0).
  _elementsToFeet(raw) {
  ...
  // as you edit, before saving (v7.29.0).
  ...
  // Per-bay garage door open/closed from the split door_mapping slots (v7.29.0).
  _house3dGarage() {
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.29.0).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.29.0): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.29.0) — rebuild it
  ...
  // ---- Floor-plan real dimensions (v7.29.0) ----
...
`;
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // clipped (v7.29.0). Drag uses getScreenCTM so it adapts to any viewBox.
    ...
    // rooms can be kept within it (v7.29.0). Enclosed rooms only (not outdoor zones).
    ...
    // placed openings as wall markers (v7.29.0)
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.29.0: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.29.0) -->
...
${this._renderRoutineLearning(d)}
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.29.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.29.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.29.0 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
      ...
      <!-- INTRUSION LOG + TRAINING (v7.29.0) -->
      <div class="panel">
        <div class="head">
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.29.0: approval now installs the automation into HA directly.
    ...
    // Routine learning: add/remove specific opt-in entities (v7.29.0)
    ...
    // Voice Confirmation: announce test (v7.29.0)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.29.0)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.29.0)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.29.0).
  ...
  _wireFloorPlanControls() {
    ...
    // Openings (windows/doors) — add / edit / remove (v7.29.0)
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.29.0)
  ...
  _wireFloorPlanDrag() {
    ...
    svgEl.addEventListener("mousemove", (e) => {
      ...
      if (dragging.resize) {
        ...
        // the garage (x<0) and in front of the home (y<0). (v7.29.0)
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.29.0): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.29.0).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.29.0): MJPEG stream → proxy stills →
        ...
        // v7.29.0: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.29.0: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v7.29.0) ──
  ...
  // ── Optional ChromaDB vector backend — v7.29.0 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.29.0).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.29.0: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.29.0: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.29.0) */
  ...
  /* AREA DETAIL DRILL-DOWN (v7.29.0) */
  .area-detail-overlay {
