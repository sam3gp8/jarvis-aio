 ...
 * v6.67.0 (session 2 · audio routing fix, areas with icons+codes)
 *
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v6.67.0)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v6.67.0 fallback chain)
    ...
    // Real-time entity subscriptions (v6.67.0) — a native state_changed feed
    ...
    // Sparklines (v6.67.0) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v6.67.0) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      config: live.config || {},
      doors: live.doors || {},
      // v6.67.0: goals card. Also fixes suggestions, which _data() never
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v6.67.0).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v6.67.0): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v6.67.0) — rebuild it
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v6.67.0: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v6.67.0) -->
  ...
  <div class="settings-page">
    ...
    <div class="settings-grid">
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v6.67.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v6.67.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v6.67.0 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v6.67.0: approval now installs the automation into HA directly.
    ...
    // Voice Confirmation: announce test (v6.67.0)
  ...
  _camName(entity) {
    // JARVIS-only display name (v6.67.0): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v6.67.0).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v6.67.0): MJPEG stream → proxy stills →
        ...
        // v6.67.0: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v6.67.0: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v6.67.0) ──
  ...
  // ── Optional ChromaDB vector backend — v6.67.0 ──
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v6.67.0: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v6.67.0: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v6.67.0) */
  ...
  /* AREA DETAIL DRILL-DOWN (v6.67.0) */
  .area-detail-overlay {
