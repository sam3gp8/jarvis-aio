 ...
 * v7.2.2 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.2.2) ----------
  ...
  // ----- generalized roofs over the derived footprint (v7.2.2) -----
  ...
  // editor's footprint + rooms + home type (v7.2.2).
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.2.2)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.2.2)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.2.2 fallback chain)
    ...
    // Real-time entity subscriptions (v7.2.2) — a native state_changed feed
    ...
    // Sparklines (v7.2.2) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.2.2) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.2.2: goals card. Also fixes suggestions, which _data() never
  ...
  // Placed openings (windows/doors) — v7.2.2. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.2.2).
  _house3dPlan() {
    const plan = this._getFloorPlan();
    const FT = 0.2, out = {};
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.2.2).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.2.2): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.2.2) — rebuild it
  ...
  // ---- Floor-plan real dimensions (v7.2.2) ----
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // placed openings as wall markers (v7.2.2)
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.2.2: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.2.2) -->
  ...
  <div class="settings-page">
    ...
    <div class="settings-grid">
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.2.2) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.2.2) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.2.2 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.2.2: approval now installs the automation into HA directly.
    ...
    // Voice Confirmation: announce test (v7.2.2)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.2.2)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.2.2)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.2.2).
  ...
  _wireFloorPlanControls() {
    ...
    // Openings (windows/doors) — add / edit / remove (v7.2.2)
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.2.2)
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.2.2): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.2.2).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.2.2): MJPEG stream → proxy stills →
        ...
        // v7.2.2: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.2.2: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v7.2.2) ──
  ...
  // ── Optional ChromaDB vector backend — v7.2.2 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.2.2).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.2.2: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.2.2: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.2.2) */
