 ...
 * v7.2.1 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.2.1) ----------
  ...
  // ----- generalized roofs over the derived footprint (v7.2.1) -----
  ...
  // editor's footprint + rooms + home type (v7.2.1).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    (plan['1f'] || []).forEach(function (r) {
      ...
      if (nm.indexOf('garage') >= 0) { garageRoom = r; return; }
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.2.1)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.2.1)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.2.1 fallback chain)
    ...
    // Real-time entity subscriptions (v7.2.1) — a native state_changed feed
    ...
    // Sparklines (v7.2.1) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.2.1) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.2.1: goals card. Also fixes suggestions, which _data() never
  ...
  // Placed openings (windows/doors) — v7.2.1. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.2.1).
  _house3dPlan() {
    const plan = this._getFloorPlan();
    const FT = 0.2, out = {};
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.2.1).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.2.1): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.2.1) — rebuild it
  ...
  _renderHouse3d() {
    ...
    const key = floor + '|' + JSON.stringify(spec) + '|' + JSON.stringify(plan);
    if (this._house3dBoxKey !== key) {
      this._house3dBox = JARVIS3D.fixedBox({ floor, spec, plan });
      this._house3dBoxKey = key;
    }
    mount.innerHTML = JARVIS3D.renderSVG({
      theta: this._house3dTheta, floor, lit: this._house3dLit(), doors: this._house3dDoors(), box: this._house3dBox, spec, plan
    });
    const d = this._data();
  ...
  // ---- Floor-plan real dimensions (v7.2.1) ----
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // placed openings as wall markers (v7.2.1)
    ...
    if (_els.length && floorData.rooms && floorData.rooms.length) {
      ...
      _els.forEach(function (e) {
        ...
        svg += '<rect x="' + ex + '" y="' + ey + '" width="' + ew + '" height="' + eh + '" fill="' + col + '" opacity="0.9" rx="1" pointer-events="none"/>';
      });
    }
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.2.1: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.2.1) -->
  ...
  <div class="settings-page">
    ...
    <div class="settings-grid">
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.2.1) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.2.1) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.2.1 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.2.1: approval now installs the automation into HA directly.
    ...
    // Voice Confirmation: announce test (v7.2.1)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.2.1)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.2.1)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.2.1).
  ...
  _wireFloorPlanControls() {
    ...
    // Openings (windows/doors) — add / edit / remove (v7.2.1)
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.2.1): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.2.1).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.2.1): MJPEG stream → proxy stills →
        ...
        // v7.2.1: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.2.1: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v7.2.1) ──
  ...
  // ── Optional ChromaDB vector backend — v7.2.1 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.2.1).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.2.1: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.2.1: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.2.1) */
