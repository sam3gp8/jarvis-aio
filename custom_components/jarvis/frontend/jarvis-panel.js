 ...
 * v7.11.0 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.11.0) ----------
  ...
  // ----- generalized roofs over the derived footprint (v7.11.0) -----
  ...
  function dormersOn(L, GL, roof, count, wSt, rear, positions) {
    wSt = wSt || 'off';
    ...
    for (i = 0; i < nD; i++) {
      ...
      winY(L, GL, yFace, cx - 1.7, cx + 1.7, zSill + 0.3, zHead - 0.3, wSt, true, out);
    }
  }
  ...
  // the house and low at the outer edge, split into two door panels (v7.11.0).
  ...
  // A door on a footprint wall, open (swung) or closed, for placed exterior/cellar doors (v7.11.0).
  ...
  // editor's footprint + rooms + home type (v7.11.0).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    extWalls(L, minx, miny, maxx, maxy, 0, eave);
    ...
    var placed = ((opts.elements && opts.elements['1f']) || []).filter(function (e) { return e.type === 'window' || (e.type === 'door' && e.kind !== 'interior'); });
    ...
    var roof, gLow = (rt === 'gable' && garageRoom && garageRoom.w > 8);
    if (rt === 'flat') roof = flatRoofOver(L, GL, minx, miny, maxx, maxy, ztop);
      ...
      var gLeft = Math.abs(garageRoom.x - minx) <= Math.abs(maxx - (garageRoom.x + garageRoom.w));
      var splitX = gLeft ? garageRoom.x + garageRoom.w : garageRoom.x;
      var hx0 = gLeft ? splitX : minx, hx1 = gLeft ? maxx : splitX;
      var gx0 = gLeft ? minx : splitX, gx1 = gLeft ? splitX : maxx;
      roof = gableRoofOver(L, GL, hx0, miny, hx1, maxy, eave, minRise);
      gableRoofOver(L, GL, gx0, miny, gx1, maxy, _oneFloorTop(), 0, 0.45);
    ...
    if (rt !== 'flat') {
      ...
      if (dormF.length) dormersOn(L, GL, roof, 0, d2, false, dormF.map(function (e) { return e.pos != null ? e.pos : 0.5; }));
      else if (SPEC.dormersFront) dormersOn(L, GL, roof, SPEC.dormersFront, d2, false);
      if (dormR.length) dormersOn(L, GL, roof, 0, d2, true, dormR.map(function (e) { return e.pos != null ? e.pos : 0.5; }));
      else if (SPEC.dormersRear) dormersOn(L, GL, roof, SPEC.dormersRear, d2, true);
    }
    ...
    ((opts.elements && opts.elements['2f']) || []).filter(function (e) { return e.type === 'window'; }).forEach(function (e) {
      var p = e.pos != null ? e.pos : 0.5, w = e.w || 4, z2 = 9.4, z2t = z2 + 3.2;
      if (e.wall === 'left' || e.wall === 'right') {
        var xg = e.wall === 'left' ? ((h2.x0 - minx < 3) ? minx : h2.x0) : ((maxx - h2.x1 < 3) ? maxx : h2.x1), yc = h2.y0 + p * (h2.y1 - h2.y0);
        winX(L, GL, xg, yc - w / 2, yc + w / 2, z2, z2t, d2, true, e.wall === 'left' ? -0.06 : 0.06);
      } else {
        var cx2 = h2.x0 + p * (h2.x1 - h2.x0), yf = e.wall === 'front' ? h2.y0 + 3.5 : h2.y1 - 3.5;
        winY(L, GL, yf, cx2 - w / 2, cx2 + w / 2, z2, z2t, d2, true, e.wall === 'front' ? -0.3 : 0.3);
      }
    });

    // basement exterior openings — walkout doors / egress windows at grade (v7.11.0)
    var hb = null;
    ...
    var placedB = ((opts.elements && opts.elements['bsmt']) || []).filter(function (e) { return e.type === 'window' || (e.type === 'door' && e.kind !== 'interior'); });
  ...
  function buildFromPlan(opts) {
    ...
    var draw = function (fk) {
      var z = _planZ(fk);
      (plan[fk] || []).forEach(function (r) { roomBox(L, LBL, r.x, r.y, r.w, r.d, z[0], z[1], r.label || r.name, stOf(r.name)); });
    };
    if (floor !== 'all') {
      var pf = _planFloorKey(plan, floor);
      draw(pf);
      // interior doors on this floor's rooms, open/closed from their sensor (v7.11.0)
      var zf = _planZ(pf);
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.11.0)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.11.0)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.11.0 fallback chain)
    ...
    // Real-time entity subscriptions (v7.11.0) — a native state_changed feed
    ...
    // Sparklines (v7.11.0) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.11.0) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.11.0: goals card. Also fixes suggestions, which _data() never
  ...
  // Placed openings (windows/doors) — v7.11.0. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.11.0).
  _planToFeet(plan) {
    const FT = 0.2, out = {};
    Object.keys(plan || {}).forEach(fk => {
      out[fk] = (((plan[fk] || {}).rooms) || []).map(r => ({
  ...
  // Placed openings -> feet, with open/closed resolved from each mapped sensor (v7.11.0).
  _elementsToFeet(raw) {
  ...
  // as you edit, before saving (v7.11.0).
  _renderEditorPreview() {
    try {
      ...
      return JARVIS3D.renderSVG({ theta: 0.62, floor: 'all', lit: this._house3dLit(), spec, plan, elements, garage, box });
  ...
  // Per-bay garage door open/closed from the split door_mapping slots (v7.11.0).
  _house3dGarage() {
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.11.0).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.11.0): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.11.0) — rebuild it
  ...
  _trackerOptions(selected) {
    ...
    if (selected && !cands.includes(selected)) cands.unshift(selected);
    const opts = cands.map(eid => `<option value="${this._esc(eid)}"${eid === selected ? ' selected' : ''}>${this._esc((states[eid].attributes && states[eid].attributes.friendly_name) || eid)}</option>`).join('');
  ...
  // ---- Floor-plan real dimensions (v7.11.0) ----
  ...
  _renderFloorPlanEditor(d) {
    ...
    html += '<div class="fp-editor-canvas" id="fp-editor-canvas" style="min-height:440px;">';
  ...
  _optInEntityOptions() {
    ...
    return `<option value="">\u2014 pick an entity \u2014</option>` + cands.map(eid =>
      `<option value="${this._esc(eid)}">${this._esc((states[eid].attributes && states[eid].attributes.friendly_name) || eid)}</option>`).join('');
  }
  _plList() {
  ...
  _refreshPlChips() {
    const wrap = this.shadowRoot && this.shadowRoot.querySelector('#pl-chips');
    if (!wrap) return;
    const incl = this._plList();
    wrap.innerHTML = incl.length
    ...
    wrap.querySelectorAll('.pl-del').forEach(b => b.addEventListener('click', () => {
    ...
    return `
      <div class="panel">
        ...
        <div class="pl-entities">
          ...
          <div class="pl-add"><select id="pl-entity-select" class="op-field">${this._optInEntityOptions()}</select><button class="ctrl" id="pl-add-entity">+ Add</button></div>
...
`;
  ...
  _renderOpenings(floor) {
    ...
    const rsel = (e, i) => '<select class="op-field op-room" data-op="room" data-i="' + i + '" title="room"><option value="">\u2014 room \u2014</option>' + rooms.map(r => '<option value="' + this._esc(r.name) + '"' + (e.room === r.name ? ' selected' : '') + '>' + this._esc(r.name) + '</option>').join('') + '</select>';
    const rows = els.map((e, i) => {
      const isD = e.type === 'dormer';
      const t = isD ? (e.slope === 'rear' ? 'REAR DORMER' : 'FRONT DORMER') : (e.type === 'window' ? 'WINDOW' : (e.kind === 'interior' ? 'INT DOOR' : (e.kind === 'cellar' ? 'CELLAR' : 'EXT DOOR')));
      const place = isD
        ...
        : (e.kind === 'interior' ? (rsel(e, i) + ' ' + wsel(e, i)) : wsel(e, i));
      const kc = isD ? ' op-dormer' : (e.kind === 'interior' ? ' op-int' : (e.kind === 'cellar' ? ' op-cellar' : ''));
      return '<div class="op-row" data-i="' + i + '">'
        ...
        + ' <select class="op-field op-ent" data-op="entity" data-i="' + i + '" title="open/closed sensor">' + this._doorEntityOptions(e.entity || '') + '</select>'
    ...
    return '<div class="fp-openings">'
      + '<div class="op-headr">OPENINGS \u00b7 windows, doors &amp; dormers <span class="op-hint">interior doors attach to a room \u00b7 dormers are placed on the 2nd floor \u00b7 map each to a sensor</span></div>'
      + '<div class="op-add"><button class="ctrl" id="op-add-window">+ Window</button><button class="ctrl" id="op-add-extdoor">+ Exterior Door</button><button class="ctrl" id="op-add-cellar">+ Cellar Door</button><button class="ctrl" id="op-add-intdoor">+ Interior Door</button>' + dBtns + '</div>'
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // placed openings as wall markers (v7.11.0)
    var _els = (this._getEditingElements()[floor]) || [];
    if (_els.length && floorData.rooms && floorData.rooms.length) {
      var mnx = 1e9, mny = 1e9, mxx = -1e9, mxy = -1e9;
      floorData.rooms.forEach(function (r) { mnx = Math.min(mnx, r.x); mny = Math.min(mny, r.y); mxx = Math.max(mxx, r.x + r.w); mxy = Math.max(mxy, r.y + r.h); });
      _els.forEach(function (e, i) {
        ...
        var col = e.type === 'window' ? '#00f2fe' : (e.kind === 'interior' ? '#5a7a8a' : (e.kind === 'cellar' ? '#c98a2a' : '#ffaa28'));
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.11.0: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.11.0) -->
...
${this._renderRoutineLearning(d)}
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.11.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.11.0) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.11.0 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
      ...
      <!-- INTRUSION LOG + TRAINING (v7.11.0) -->
      <div class="panel">
        <div class="head">
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.11.0: approval now installs the automation into HA directly.
    ...
    // Routine learning: add/remove specific opt-in entities (v7.11.0)
    const _plAddBtn = this.shadowRoot.querySelector('#pl-add-entity');
    if (_plAddBtn) _plAddBtn.addEventListener('click', () => {
      const sel = this.shadowRoot.querySelector('#pl-entity-select');
      if (sel && sel.value) { const arr = this._plList(); if (!arr.includes(sel.value)) { arr.push(sel.value); this._plSave(arr); } sel.value = ''; }
    });
    ...
    // Voice Confirmation: announce test (v7.11.0)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.11.0)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.11.0)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.11.0).
  ...
  _wireFloorPlanControls() {
    ...
    if (fpSave) {
      fpSave.addEventListener("click", async () => {
        if (!this._editingPlan && !this._editingElements) return;
        const savedPlan = this._editingPlan, savedEls = this._editingElements;
        try {
          ...
          this._editingPlan = null; this._editingElements = null;
    ...
    // Openings (windows/doors) — add / edit / remove (v7.11.0)
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.11.0)
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.11.0): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.11.0).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.11.0): MJPEG stream → proxy stills →
        ...
        // v7.11.0: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.11.0: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v7.11.0) ──
  ...
  // ── Optional ChromaDB vector backend — v7.11.0 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.11.0).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.11.0: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.11.0: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.11.0) */
  ...
  /* AREA DETAIL DRILL-DOWN (v7.11.0) */
  .area-detail-overlay {
