 ...
 * v7.3.1 (session 2 · audio routing fix, areas with icons+codes)
 *
...
const JARVIS3D = (function () {
  ...
  // ---------- data-driven build: geometry from the editor's rooms (feet) (v7.3.1) ----------
  ...
  // ----- generalized roofs over the derived footprint (v7.3.1) -----
  ...
  function dormersOn(L, GL, roof, count, wSt, rear) {
    wSt = wSt || 'off';
  ...
  function garageDoorsOn(L, GL, x0, x1, yF, state) {
    var bays = SPEC.garageBays > 0 ? SPEC.garageBays : 3, gap = 1.8;
  ...
  // the house and low at the outer edge, split into two door panels (v7.3.1).
  function bulkheadDoor(L, GL, wall, cx, cy, w, open) {
    var depth = Math.max(w * 0.95, 5), zHigh = 4.4, zLow = 0.3;
  ...
  // A door on a footprint wall, open (swung) or closed, for placed exterior/cellar doors (v7.3.1).
  ...
  // editor's footprint + rooms + home type (v7.3.1).
  function buildExteriorFromPlan(opts, plan, minx, miny, maxx, maxy, ztop) {
    ...
    if (garageRoom) {
      ...
      garageDoorsOn(L, GL, g.x + 0.5, g.x + g.w - 0.5, gy, wOf(g.name));
    }
    ...
    if (rt !== 'flat' && SPEC.dormersFront) dormersOn(L, GL, roof, SPEC.dormersFront, d2, false);
    if (rt !== 'flat' && SPEC.dormersRear) dormersOn(L, GL, roof, SPEC.dormersRear, d2, true);
    ...
    ((opts.elements && opts.elements['2f']) || []).filter(function (e) { return e.type === 'window'; }).forEach(function (e) {
      ...
      if (e.wall === 'left' || e.wall === 'right') {
        var xg = e.wall === 'left' ? minx : maxx, yc = miny + p * (maxy - miny);
        winX(L, GL, xg, yc - w / 2, yc + w / 2, z2, z2t, d2, true, e.wall === 'left' ? -0.06 : 0.06);
      } else {
        var cx2 = minx + p * (maxx - minx), yf = e.wall === 'front' ? miny + 3.5 : maxy - 3.5;
        winY(L, GL, yf, cx2 - w / 2, cx2 + w / 2, z2, z2t, d2, true, e.wall === 'front' ? -0.3 : 0.3);
      }
    });

    if (!placed.length && plan['bsmt'] && plan['bsmt'].length) {
...
class JarvisPanel extends HTMLElement {
  constructor() {
    ...
    this._logSearch = "";          // log text search (v7.3.1)
    ...
    this._editingElements = null; // working copy of placed windows/doors (v7.3.1)
    ...
    this._camMode = "stream";      // stream → still → jarvis (v7.3.1 fallback chain)
    ...
    // Real-time entity subscriptions (v7.3.1) — a native state_changed feed
    ...
    // Sparklines (v7.3.1) — slow-polled separately from live data since
    // recorder history queries are heavier than the rest of the payload.
    this._sparklines = {};
    this._sparklineInterval = null;
    // Area drill-down (v7.3.1) — id of the area currently expanded, or null.
  ...
  _data() {
    ...
    return {
      ...
      doors: live.doors || {},
      // v7.3.1: goals card. Also fixes suggestions, which _data() never
  ...
  // Placed openings (windows/doors) — v7.3.1. Parallel to the room plan.
  _getFloorElements() {
  ...
  // house geometry is built from the floor plan (FT_PER_UNIT = 0.2) (v7.3.1).
  _house3dPlan() {
    const plan = this._getFloorPlan();
    const FT = 0.2, out = {};
  ...
  // Placed openings -> feet, with open/closed resolved from each mapped sensor (v7.3.1).
  _house3dElements() {
    const raw = this._getFloorElements();
    ...
    Object.keys(raw || {}).forEach(fk => {
      out[fk] = (raw[fk] || []).map(e => {
        ...
        return { type: e.type, kind: e.kind, wall: e.wall, room: e.room, pos: (e.pos != null ? e.pos : 0.5), w: (e.w || 20) * FT, open };
      });
    });
    return out;
  }
  ...
  // (dominant room). mmWave overlays on top of plain occupancy (v7.3.1).
  _house3dLit() {
    const d = this._data();
    const lit = {};
  ...
  // mmWave presence overview (v7.3.1): live per-room sensor state, fetched
  ...
  async _fetchMmwave() {
    ...
    this._renderMmwave();
    // Fresh mmWave state feeds the floor-plan glow too (v7.3.1) — rebuild it
  ...
  _renderHouse3d() {
    ...
    const key = floor + '|' + JSON.stringify(spec) + '|' + JSON.stringify(plan) + '|' + JSON.stringify(elements);
    if (this._house3dBoxKey !== key) {
      this._house3dBox = JARVIS3D.fixedBox({ floor, spec, plan, elements });
      this._house3dBoxKey = key;
    }
    mount.innerHTML = JARVIS3D.renderSVG({
      theta: this._house3dTheta, floor, lit: this._house3dLit(), doors: this._house3dDoors(), box: this._house3dBox, spec, plan, elements
    });
    const d = this._data();
  ...
  // ---- Floor-plan real dimensions (v7.3.1) ----
  ...
  _renderOpenings(floor) {
    ...
    const rows = els.map((e, i) => {
      const t = e.type === 'window' ? 'WINDOW' : (e.kind === 'interior' ? 'INT DOOR' : (e.kind === 'cellar' ? 'CELLAR' : 'EXT DOOR'));
      const place = e.kind === 'interior' ? (rsel(e, i) + ' ' + wsel(e, i)) : wsel(e, i);
      const kc = e.kind === 'interior' ? ' op-int' : (e.kind === 'cellar' ? ' op-cellar' : '');
      return '<div class="op-row" data-i="' + i + '">'
        + '<span class="op-type op-' + e.type + kc + '">' + t + '</span>'
        + place
        ...
        + ' <input class="op-field" data-op="w" data-i="' + i + '" type="number" min="1" step="0.5" value="' + this._fpToReal(e.w || 20) + '" style="width:46px"> ' + uL
    ...
    return '<div class="fp-openings">'
      + '<div class="op-headr">OPENINGS \u00b7 windows &amp; doors on this floor <span class="op-hint">interior doors attach to a room \u00b7 map each to a sensor for open/closed</span></div>'
      + '<div class="op-add"><button class="ctrl" id="op-add-window">+ Window</button><button class="ctrl" id="op-add-extdoor">+ Exterior Door</button><button class="ctrl" id="op-add-cellar">+ Cellar Door</button><button class="ctrl" id="op-add-intdoor">+ Interior Door</button></div>'
  ...
  _renderEditableSVG(plan, floor) {
    ...
    // placed openings as wall markers (v7.3.1)
  ...
  _html() {
    ...
    const areaTile = (a) => {
      ...
      const lightCtl = hasLights
        ...
        : '';
      // v7.3.1: temp/humidity readout + sparkline, when the area has a sensor.
      const spark = this._sparklines?.[a.id] || {};
      const tempSpark = spark.temp ? this._sparklineSvg(spark.temp, 'var(--cyan-dim)') : '';
      const humSpark = spark.humidity ? this._sparklineSvg(spark.humidity, 'var(--green)') : '';
...
<div class="app">
  ...
  <div class="res-tab">
    ...
    <!-- mmWave presence overview (v7.3.1) -->
  ...
  <div class="settings-page">
    ...
    <div class="settings-grid">
      ...
      <div class="panel">
        ...
        <div class="home-cfg">
          ...
          <div class="cfg-row">
            ...
            <input class="cfg-field cfg-text" type="text" data-cfg-key="departure_origin_entity" value="${this._esc(d.config?.departure_origin_entity || '')}" placeholder="person.you (optional)">
          </div>
          <div class="cfg-row">
          ...
          <div class="cfg-row">
            ...
            <input class="cfg-field cfg-text" type="text" data-cfg-key="departure_travel_sensor" value="${this._esc(d.config?.departure_travel_sensor || '')}" placeholder="sensor.commute (optional)">
          </div>
      ...
      <!-- OPERATIONAL MODE (Directive Layer, v7.3.1) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- JARVIS CHARACTER + RESEARCH (v7.3.1) -->
      <div class="panel">
        <div class="head">
      ...
      <!-- CAMERAS (names + location designation, v7.3.1 — moved from Command Center) -->
      <div class="panel">
        <div class="head">
          <span>Cameras</span>
  ...
  _wire() {
    ...
    this.shadowRoot.querySelectorAll(".sug").forEach(card => {
      ...
      const act = async (action) => {
        ...
        try {
          ...
          if (action === "approve") {
            // v7.3.1: approval now installs the automation into HA directly.
    ...
    // Voice Confirmation: announce test (v7.3.1)
    ...
    // Onboarding welcome card: dismiss + settings jump (v7.3.1)
    ...
    // Per-step jump: switch to Settings and scroll/flash the relevant card (v7.3.1)
  ...
  // floor switching, Add Room, delete, etc. keep working after each update (v7.3.1).
  ...
  _wireFloorPlanControls() {
    ...
    // Openings (windows/doors) — add / edit / remove (v7.3.1)
    ...
    // Highlight an opening's marker while you hover its row or pick its sensor (v7.3.1)
  ...
  _camName(entity) {
    // JARVIS-only display name (v7.3.1): camera_names map → picker name →
  ...
  _renderCameraFeed() {
    ...
    // or token changes. src = the frame source (override-aware, v7.3.1).
    ...
    if (key !== this._lastCamKey) {
      ...
      if (!img) {
        ...
        feed.prepend(img);
        // Escalating fallback chain (v7.3.1): MJPEG stream → proxy stills →
        ...
        // v7.3.1: a decoded frame proves the tier works only if it isn't
      ...
      if (this._camModeByEntity[entity] === "jarvis") {
        ...
        // v7.3.1: no-frame watchdog. Nest WebRTC proxies typically HANG
  ...
  // ── System Diagnostics — core service health (v7.3.1) ──
  ...
  // ── Optional ChromaDB vector backend — v7.3.1 ──
  ...
  _wireCameraSettings() {
    // Camera on/off toggles — choose which cameras JARVIS uses (v7.3.1).
  ...
  _armCamWatchdog(entity, img, expectMode, ms) {
    ...
    this._camWatchdog = setTimeout(() => {
      ...
      // v7.3.1: pixels alone don't prove a working tier — a Nest MJPEG can
  ...
  _camJarvisFallback(entity) {
    ...
    const shot = async () => {
      ...
      try {
        ...
        // v7.3.1: don't swallow this — the most common cause is the WS
...
<style>
  ...
  *::-webkit-scrollbar-track { background: transparent; }
  ...
  /* AREA READINGS + SPARKLINES (v7.3.1) */
